...
try:
    ...
    def hash(password, salt):
        kdf = Scrypt(salt=salt, length=SCRYPT_LEN, n=SCRYPT_N, r=SCRYPT_R, p=SCRYPT_P, backend=default_backend())
    ...
    try:
        ...
        def hash(password, salt):
            return shash(
                password=password.encode("utf-8"), salt=salt, N=SCRYPT_N, r=SCRYPT_R, p=SCRYPT_P, dkLen=SCRYPT_LEN
            )
        ...
        def hash(password, salt):
            return shash(
                password=password.encode("utf-8"), salt=salt, N=SCRYPT_N, r=SCRYPT_R, p=SCRYPT_P, buflen=SCRYPT_LEN
            )
...
CHARACTER_CLASSES = {
    ...
    "n": "0123456789",
    "o": "@&%?,=[]_:-+*$#!'^~;()/.",
    "x": "AEIOUaeiouBCDFGHJKLMNPQRSTVWXYZ" "bcdfghjklmnpqrstvwxyz0123456789!@#$%^&*()",
}
...
class MasterPassword:
    ...
    def derive(self, type, site, counter=1):
        ...
        for i in range(0, len(template)):
            pass_chars = CHARACTER_CLASSES[template[i]]
            pass_char = pass_chars[seed[i + 1] % len(pass_chars)]
            value += pass_char
